...
export class AuthService {
  private emailVerificationStore = new Map<string, EmailVerificationState>();
  private verificationTtlMs = 10 * 60 * 1000;
...
<body style="margin:0;padding:0;background-color:#f4f4f7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  ...
  async verifyEmailCode(data: VerifyEmailCodeDTO): Promise<{ message: string; verified: boolean }> {
    const email = this.normalizeEmail(data.email);
    const state = this.emailVerificationStore.get(email);
    if (!state) throw new AppError(400, 'Please request a verification code first');
    ...
    if (state.code !== data.code.trim()) throw new AppError(400, 'Invalid verification code');
  ...
  private async createSession(
    userId: string,
    token: string,
    meta: { ipAddress: string; userAgent: string; browserId: string }
  ): Promise<void> {
